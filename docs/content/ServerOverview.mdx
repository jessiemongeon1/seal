...
- Accepts decryption requests for any onchain package without restrictions.
...
- **Decentralized key server**: See [Key Server Operations for Decentralized Server Type](/KeyServerCommitteeOps) for detailed setup instructions including DKG ceremonies, key rotation, and key server setup.
