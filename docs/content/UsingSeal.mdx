...
Read the [Seal Design document](/Design) first to understand the underlying architecture and concepts before using this guide.
...
Building and publishing the code can be done using the [Sui CLI](https://docs.sui.io/references/cli), for example:
```shell
cd examples/move
sui move build
sui client publish
...
You can obtain an API key from [Enoki](https://enoki.mystenlabs.com/). The header name is always `X-API-Key`.
...
See the [Pricing](/Pricing#verified-key-servers) page for the verified key server object IDs and aggregator URLs.
...
- The ID associated with the access control policy (without the prefix of the package ID discussed in [Seal Design](/Design))
- The data to encrypt
...
Seal supports onchain HMAC-CTR decryption in Move through the [`seal::bf_hmac_encryption`](https://github.com/MystenLabs/seal/tree/main/move/seal/sources/bf_hmac_encryption.move) package. This enables Move packages to decrypt Seal-encrypted objects and use the results in onchain logic such as auctions, secure voting (see [voting.move](https://github.com/MystenLabs/seal/tree/main/move/patterns/sources/voting.move)), or other verifiable workflows.
...
```move
// DO: derived keys come from the caller, public keys come from the package.
public fun reveal(obj: &EncryptedObject, derived_keys: &vector<vector<u8>>): Option<vector<u8>>

// DON'T: the caller picks the keys, so the caller picks the plaintext.
public fun reveal(obj: &EncryptedObject, derived_keys: &vector<vector<u8>>, public_keys: vector<vector<u8>>): Option<vector<u8>>
```

See [`patterns::voting`](https://github.com/MystenLabs/seal/blob/main/move/patterns/sources/voting.move), where `finalize_vote` reads the public keys from the `Vote` object they were fixed in.
